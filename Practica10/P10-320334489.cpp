...
float reproduciranimacion, habilitaranimacion, guardoFrame, reinicioFrame, ciclo, ciclo2, contador = 0, cicloy, cicloy2, ciclor, ciclor2, cicloS, cicloS2;
...
void writeKeyframesToFile(const std::string& filename)
{
	...
	printf("Se escribieron %d keyframes en '%s'.\n", FrameIndex, filename.c_str());
}
...
void loadKeyframesFromFile(const std::string& filename) {
	...
	if (!file.is_open()) {
		printf("No se encontró '%s'. Creando un nuevo archivo con el frame inicial...\n", filename.c_str());
		std::ofstream createFile(filename);
		if (createFile.is_open()) {
			createFile << "0.0 0.0 0.0 0.3\n";
			createFile.close();

			KeyFrame[0].movAvion_x = 0.0f;
			KeyFrame[0].movAvion_y = 0.0f;
			KeyFrame[0].giroAvion = 0.0f;
			KeyFrame[0].scale = 0.3f;
			FrameIndex = 1;

			printf("Archivo '%s' creado con el frame inicial.\n", filename.c_str());
			return;
		}
		else {
	...
	FrameIndex = 0;
	FRAME temp;
	while (file >> temp.movAvion_x >> temp.movAvion_y >> temp.giroAvion >> temp.scale) {
		KeyFrame[FrameIndex++] = temp;
		if (FrameIndex >= MAX_FRAMES) break;
	}
	file.close();
	if (FrameIndex == 0) {
		printf("El archivo estaba vacío. Se agrega el frame inicial.\n");
		std::ofstream createFile(filename);
		if (createFile.is_open()) {
			createFile << "0.0 0.0 0.0 0.3\n";
			createFile.close();
...
void saveFrame(void) //tecla L
{
	...
	KeyFrame[FrameIndex].scale = scale;
	...
	FrameIndex++;
	writeKeyframesToFile("keyframes.txt");
}
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].scaleInc = (KeyFrame[playIndex + 1].scale - KeyFrame[playIndex].scale) / i_max_steps;
}
...
void animate(void)
{
	...
	if (play)
	{
		...
		else
		{
			...
			scale += KeyFrame[playIndex].scaleInc;
			i_curr_steps++;
		}
...
void mostrarMenu() {
	...
	printf("5.-Presiona 1 para Aumentar en X\n6.-Presiona 2 para Disminuir en X\n7.-Presiona 3 para Habilitar mover en X\n");
	printf("9.-Presiona 4 para Girar\n10.-Presiona 5 para Habilitar Giro\n");
	printf("11.-Presiona 6 para Aumentar Escalado\n12.-Presiona 7 para Disminuir Escalado\n13.-Presiona 8 para Habilitar Escalado\n");
}
...
int main()
{
	...
	Blackhawk_M = Model();
	Blackhawk_M.LoadModel("Models/ninja_star.obj");
...
void inputKeyframes(bool* keys)
{
	...
	if (keys[GLFW_KEY_1])
	{
		if (ciclo < 1)
		{
			...
			ciclo++;
			ciclo2 = 0;
			printf("\n Presiona la tecla 3 para poder habilitar la variable\n");
		}
	...
	if (keys[GLFW_KEY_3])
	{
		if (ciclo2 < 1)
		{
			ciclo = 0;
			ciclo2++;
			printf("\n Ya puedes modificar tu variable presionando la tecla 1 o 2\n");
		}
	}
	//Giro
	if (keys[GLFW_KEY_4])
	...
	if (keys[GLFW_KEY_8])
	{
		if (cicloS2 < 1)
		{
			cicloS = 0;
			cicloS2++;
			printf("\n Ya puedes modificar tu variable presionando la tecla 7 o 6\n");
		}
	}
