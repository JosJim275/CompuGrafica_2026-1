...
int main()
{
	...
	PuertaLetrero = Model();
	PuertaLetrero.LoadModel("Models/PuertaLetrero.obj");
	...
	glm::vec3 DragonPos = glm::vec3(0.0f, 5.0f, 6.0f);
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(DragonPos.x, DragonPos.y + sin(glm::radians(angulovaria * 2.0f)) * 1.5f, DragonPos.z + sin(glm::radians(angulovaria)) * 20.0f));
		...
		//Ala Derecha
		aleteo = sin(glm::radians(angulovaria * 3.0f)) * 30.0f;
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.56f, 3.0f, 0.37f));
		model = glm::rotate(model, glm::radians(-aleteo), glm::vec3(0.0f, 0.0f, 1.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		AlaDerecha.RenderModel();
		//Ala Izquierda
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.5f, 3.4f, 0.07f));
		model = glm::rotate(model, glm::radians(aleteo), glm::vec3(0.0f, 0.0f, 1.0f));
		Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		*/
		toffsetnumerocambiau += 0.01f * deltaTime;
		...
		tiempoCambio += 0.01f * deltaTime;
		if (tiempoCambio >= 1.0f) {
		...
		model = glm::rotate(model, 90 * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		PuertaLetrero.RenderModel();

		//Puertas

		model = modelaux;
		model = glm::translate(model, glm::vec3(3.55f, 6.7f, 0.0f));
