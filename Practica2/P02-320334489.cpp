...
static const char* vShaderColorGreenF = "shaders/shadercolorGreenF.vert";
...
void CrearO() {
	...
	Mesh* O_letra = new Mesh();
	O_letra->CreateMesh(O_vertices, O_indices, 36, 24);
	meshList.push_back(O_letra);
};
...
void CrearE() {
	...
	Mesh* E_letra = new Mesh();
	E_letra->CreateMesh(E_vertices, E_indices, 45, 24);
	meshList.push_back(E_letra);
}
...
// Triangulo de diferentes colores
...
void CreateShaders()
{
	...
	//Shader color verde fosforescente
	Shader* shaderGreenF = new Shader();
	shaderGreenF->CreateFromFiles(vShaderColorGreenF, fShader);
	shaderList.push_back(*shaderGreenF);
}
...
int main()
{
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Letras con MeshColorList Para que tengan el color
		...
		// Letras con los puntos que representan vertices y los cuales se repiten se reutiliza el vertice
		...
		//Casa en 3D Utilizando piramides y cubos.
		...
		model = glm::scale(model, glm::vec3(1.5f, 0.5f, 0.15f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));//FALSE ES PARA QUE NO SEA TRANSPUESTA
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
